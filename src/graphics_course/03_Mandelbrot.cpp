...
// --- Vertex Shader ---
// This just renders a full-screen rectangle (Quad)
...
// This is where the magic happens, as requested in the slide
const char* fragmentShaderSource = R"(
    #version 460 core
    out vec4 FragColor;
    
    uniform vec2 u_resolution; // Screen size
    uniform float u_zoom;      // Zoom level (optional interaction)
    uniform vec2 u_center;     // Center position (optional interaction)
    ...
    void main() {
        ...
        vec2 z = vec2(0.0);
        int maxIter = 100; // Increase this for better detail at high zoom
        int iter = 0;
...
int main() {
    ...
    // --- Create a Full-Screen Rectangle (Quad) ---
    // Two triangles covering the screen from -1 to 1
    float vertices[] = {
        // Positions        
    ...
    // IMPORTANT: Reset PolygonMode to FILL (in case previous assignment set it to LINE)
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

    // --- Zoom/Pan State ---
    float zoom = 1.0f;
    float centerX = -0.75f; // Standard Mandelbrot center
    float centerY = 0.0f;
    ...
    while (window.isOpen()) {
        while (const std::optional event = window.pollEvent()) {
            ...
            // Optional: Simple Controls
                ...
                if (keyPressed->code == sf::Keyboard::Key::Equal) zoom *= 1.1f; // Zoom In (+)
                if (keyPressed->code == sf::Keyboard::Key::Hyphen) zoom /= 1.1f; // Zoom Out (-)
